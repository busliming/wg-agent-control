...
const prefix = require('gulp-prefix-url');
 const config=require('web-app-amina-new'); //引用配置的路径文件
...
　　　　　　　　//压缩后放在那个文件夹
        ...
        gulp.src(config.src+'img/h5/*')
        .pipe(gulp.dest(config.dev+"img/h5"))
        .pipe(connect.reload())
...
gulp.task('auto',()=>{
   ...
   gulp.watch([config.src+'img/*.{jpg,png,gif}'], ['imagemin']);
})
...
// gulp.task('testLess-prod',function(){
//     gulp.src([config.src+'css/detail.less'])//多个文件以数组形式传入
//         .pipe(less())
//         .pipe(gulp.dest(config.src+'css'))//将会在src/css下生成index.css以及detail.css
// });
...
gulp.task('imagemin-prod',  () =>{
        ...
        gulp.src(config.src+'img/h5/*')
        .pipe(gulp.dest(config.dist+"img/h5"))
});



gulp.task('build',['imagemin-prod','html-prod','minify-css-prod','script-prod','copy-prod']);
...
// let prefixUrl  = {
//     root:'http://portal.wg-dns-prod.com/portal/',
//     url:'yaojiqipai/pc/dist/'
// }  
// gulp.task('script', () => {
//     // 1. 找到文件
...
// );
// gulp.task('testLess',function(){
//     gulp.src(['src/css/detail.less'])//多个文件以数组形式传入
//         .pipe(less())
...
// 		      return [
// 		        proxy('/api', {
// 		          target: 'http://www.abe123.com:19013/',
// 		          changeOrigin:true,
// 		          pathRewrite: {
// 			          '^/api':''
// 			       }
...
// 	 });
// });
// gulp.task('gulp-server', ['auto','server','imagemin','html','minify-css','testLess','script','copy']);